...
class OpsErrorHandler {
    ...
    private val logger = LoggerFactory.getLogger(OpsErrorHandler::class.java)

    @ExceptionHandler(IllegalStateException::class)
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    fun handleDrill(ex: IllegalStateException): Map<String, Any> {
        val traceId = MDC.get("traceId") ?: "unknown"
        logger.warn("ops.event.drill traceId={} reason={}", traceId, ex.message)
        return mapOf(
            "status" to "error",
